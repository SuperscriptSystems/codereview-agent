...
vi.mock('../../src/git/filtering.js', () => ({
	...
	isGeneratedFile: isGeneratedFileMock,
...
function buildReviewConfig(overrides?: Record<string, unknown>) {
	return {
		...
		generatedFiles: {
			...
			logExcluded: true,
			ignoredPaths: ['__generated__', 'generated'],
			ignoredPatterns: [
				'.generated.',
				'generated.',
				'openapi',
...
describe('review command', () => {
	...
	beforeEach(() => {
		...
		parseConfigMock.mockReturnValue({
			review: buildReviewConfig(),
		});

		filterTestFilesMock.mockImplementation(value => value);
		isGeneratedFileMock.mockResolvedValue(false);
	...
	it('returns early when only excluded generated files remain', async () => {
		getDiffMock.mockResolvedValue('full diff text');
		parseChangedFilesFromDiffMock.mockReturnValue({
			'src/services/crm-api-schema.ts': 'schema-diff',
		});
		getCommitMessagesMock.mockResolvedValue('commit');
		isGeneratedFileMock.mockResolvedValue(true);
	...
	it('logs excluded files and batch counts', async () => {
		...
		parseChangedFilesFromDiffMock.mockReturnValue({
			'src/app.ts': 'app-diff',
			'package-lock.json': 'lock-diff',
			'src/generated/openapi.ts': 'generated-diff',
		...
		isGeneratedFileMock.mockImplementation(
			async (repoPath: string, filePath: string) =>
				filePath === 'src/generated/openapi.ts',
		);
		...
		expect(loggerFns.info).toHaveBeenCalledWith(
			'Excluded generated files: src/generated/openapi.ts',
		);
		expect(loggerFns.info).toHaveBeenCalledWith(
