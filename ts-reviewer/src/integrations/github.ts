...
async function cleanupComments(
  ...
  const reviewComments = (await api(`/repos/${owner}/${repo}/pulls/${prNumber}/comments`)) as Array<{
    id: number
    in_reply_to_id?: number
    user?: { login?: string }
  }> | null
  const issueComments = (await api(`/repos/${owner}/${repo}/issues/${prNumber}/comments`)) as Array<{
    id: number
    user?: { login?: string }
  }> | null
  const reviews = (await api(`/repos/${owner}/${repo}/pulls/${prNumber}/reviews`)) as Array<{
    id: number
    state?: string
    user?: { login?: string }
  }> | null
...
async function getLatestCommitSha(
  ...
  const commits = (await api(`/repos/${owner}/${repo}/pulls/${prNumber}/commits`)) as Array<{ sha: string }>
