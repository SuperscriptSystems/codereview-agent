...
const filterFilesByPatternMock = vi.fn()
...
vi.mock("../../src/git/filtering.js", () => ({
  ...
  filterFilesByPattern: filterFilesByPatternMock,
}))
...
describe("review command", () => {
  ...
  beforeEach(() => {
    ...
    filterFilesByPatternMock.mockImplementation((value) => value)
  ...
  it("returns early when only filtered files remain", async () => {
    getDiffMock.mockResolvedValue("full diff text")
    parseChangedFilesFromDiffMock.mockReturnValue({ "src/app.test.ts": "test-diff" })
    getCommitMessagesMock.mockResolvedValue("commit")
    filterTestFilesMock.mockReturnValue({})
